...
#' @param D.start Starting value for D. Default is NA.
#' @param P.start = NA
#' @param Waic If \code{TRUE}, compute WAIC and store as \code{Waic.out}. Default is \code{FALSE}.
#' @param marginal If \code{TRUE}, compute marginal likelihood. Default is \code{FALSE}.
#' @param fixed If \code{TRUE}, the fixed effects model is chosen as a baseline model. Default is \code{TRUE}.
#' @param unscaled.Y Unscaled response vector (used for intercept estimation when standardize is TRUE).
#' @param unscaled.X Unscaled design matrix (used for intercept estimation when standardize is TRUE).
## #' @param beta.alg
...
BridgeMixedPanel <- function(
    ...
    ## prior inputs
    if (m > 0) {
        P0 <- getFromNamespace("trans.mat.prior", "MCMCpack")(m = m, n = ntime, a = a, b = b)
        ## initial values
        P <- getFromNamespace("check.P", "MCMCpack")(P.start, m, a = a, b = b)
    } else {
        P <- P0 <- matrix(1, 1, 1)
    }
  ...
  ## pull together matrix and build MCMC object to return
    ...
    if (m > 0) {
        ...
        df.beta <- do.call(rbind, list(df.beta1, df.beta2, df.beta3))
        colnames(df.beta) <- sub("X", "beta", colnames(df.beta))
