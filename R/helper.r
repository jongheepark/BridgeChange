...
dss.compute <- function(output, y, X){
    ...
    hybrid.dss <- sapply(1:ns, function(i){adaptive.lasso.weighted(y.list[[i]], x.list[[i]], beta.hat = beta.st[i,])})
...
#         X <- X[,-1]
#     }
dss.panel <- function(output, data, formula, index, model, effect,
  ...
  for (i in 1:ns) {
    x.list[[i]] <- X[state[time.id] == i, , drop = FALSE]
    y.list[[i]] <- yhat[state[time.id] == i]
    raw.y.list[[i]] <- y[state[time.id] == i]
    cat("y and yhat correlation is ", cor(y.list[[i]], raw.y.list[[i]]), "\n")
  }

  hybrid.dss <- sapply(1:ns, function(i) {
    adaptive.lasso(y.list[[i]], x.list[[i]])
  })
...
summary.dss <- function(output, variable.names=TRUE){
    ...
    varnames <- sub("_regime1", "", attr(output, "xnames")[,1])
...
## Adaptive for each regime
adaptive.lasso.weighted <- function(y1, x1, beta.hat){
    ## fit.ridge <- cv.glmnet(y = y, x = x, type.measure="mse",
    ##                        alpha=0, standardize = TRUE, family="gaussian")
...
"z.latent.internal2" <-function(Y, mu){
  ...
  right = rep(0, N);
  right[Y==1]= Inf;
  z = truncnorm::rtruncnorm(N, a = left, b = right, mean = mu, sd = 1.0);
  return(z)
}
...
#' @param outlist list of \code{BridgeChange} objects, typically outputs from \code{BridgeChangeReg}.
#' @param print boolean; if \code{TRUE} selected model is printed.
#' @return A data.frame of WAIC values.
#' @export
WaicCompare <- function(outlist, print = TRUE){
    ...
    if(isTRUE(print)) {
      cat("\n")
      cat("Selected model = break", unlist(breaks)[which.min(out$waic)],'\n')
      cat("\n")
      print.default(out$waic, quote = FALSE, digits = 5)
    }
    return(out)
}

#' Draw a plot of WAIC
#' @param out WaicCompare output.
#' @param xlab Label for x-axis.
#' @param ylab Label for y-axis.
#' @param main Plot title.
#' @return R base plot.
#' @export
plotWaic <- function(out, xlab="Model", ylab="WAIC", main="WAIC Comparison"){
    plot(out$waic, type="o", axes=FALSE, xlab=xlab, ylab=ylab, main=main, pch=19)
    axis(2)
    axis(1, at=1:3, labels=out$model)
}
...
## Find break point from posterior state probabilities
...
## Break point loss for model comparison
BreakPointLoss <- function(model.list, display = TRUE) {
    M <- length(model.list)
    ...
    model.names <- paste0("break ", break.number)

    State <- Tau <- Tau.samp <- as.list(rep(NA, M))
    for (i in 1:M) {
        State[[i]] <- attr(model.list[[i]], "s.store")
        Tau[[i]] <- findBreakPoint(model.list[[i]])
        Tau.samp[[i]] <- sapply(1:nrow(State[[i]]), function(j) {
            which(diff(State[[i]][j, ]) == 1) + 1
        })
    }
    ave.loss <- rep(NA, M)
    for (i in 1:M) {
    ...
    if (display) {
        plot(ave.loss, type = "o", xlab = "Model", ylab = "Loss",
             main = "Average Loss", axes = FALSE)
        axis(1, at = break.number, labels = model.names)
        axis(2)
        abline(v = which.min(ave.loss), lty = 3, col = "red")
    }
    return(list(ave.loss = ave.loss, State = State,
                Tau = Tau, Tau.samp = Tau.samp))
}
...
.bc_prepare_panel <- function(formula, data, index, model, effect) {
  data <- data.frame(data, stringsAsFactors = FALSE)

  vars_needed <- unique(c(all.vars(formula), index))
  ...
  if (nrow(data) == 0) {
    stop("No complete observations remain after removing missing values in formula/index variables.")
  }

  if (anyNA(data[, index, drop = FALSE])) {
    stop("Missing values remain in panel index variables after filtering.")
  }
  ...
  X <- stats::model.matrix(pm)
  y <- plm::pmodel.response(pm)
  idx <- plm::index(pm)
  ...
  if ("(Intercept)" %in% colnames(X) && identical(model, "pooling")) {
    X <- X[, colnames(X) != "(Intercept)", drop = FALSE]
  }

  if (nrow(X) != length(y)) {
    stop(sprintf(
      "Row mismatch after plm transform: nrow(X) = %s, length(y) = %s",
      nrow(X), length(y)
    ))
  }
  ...
  if (nrow(X) != nrow(idx)) {
    stop(sprintf(
      "Index mismatch after plm transform: nrow(X) = %s, nrow(index) = %s",
      nrow(X), nrow(idx)
    ))
  }
