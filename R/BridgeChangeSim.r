...
#' @param standardize If TRUE, data are standardized. Default is TRUE.
...
#' @param positive.jump If TRUE, breaks have positive jumps only.
#' @param n.break Number of structural breaks. Default is 1.
#' @param intercept If TRUE, include intercept. Default is FALSE.
#' @param positive.jump.tune Tuning parameter for positive jump size.
#' @param mcmc Number of MCMC iterations. Default is 100.
#' @param burn Number of burn-in iterations. Default is 100.
#' @param verbose Print progress every verbose-th iteration. Default is 100.
#' @param thin Thinning interval.
#' @param known.alpha If TRUE, alpha is fixed. Default is FALSE.
#' @param dgp.only If TRUE, only data are generated and estimation steps are skipped.
#' @return A list containing generated data and optionally model fit results.
#'
#' @export
BridgeChangeSim <- function(ntime=500, predictor = 100, rho=0, time.series=FALSE,
    ...
    if(rho != 0 & time.series){
        ...
        x <- t(MASS::mvrnorm(predictor, rep(0, ntime), tmp.r))
    }
    if(rho != 0 & !time.series){
