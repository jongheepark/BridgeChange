...
"BridgeChangeRegHybrid" <- function(y, X,                                             # inputs
    ...
    if (n.break > 0) P <-  getFromNamespace("trans.mat.prior", "MCMCpack")(m=m, n=ntime, a=0.9, b=0.1)   ## very quick change
    if (n.break > 0) A0 <- getFromNamespace("trans.mat.prior", "MCMCpack")(m=m, n=ntime, a=a, b=b)
    ...
    if(n.break > 0){
        ...
        if(sum(lapply(y.list, length) < regime.duration.min )>0){
            ...
            if(ols.weight){
                ...
                hybrid.dss <- sapply(which(lapply(y.list, length)>=regime.duration.min),
                                    function(i){adaptive.lasso.weighted(y.list[[i]], x.list[[i]], beta.hat = beta.st[i,])})
                rownames(hybrid.dss) <- colnames(X)
                colnames(hybrid.dss) <- paste0("Regime", which(lapply(y.list, length)>=regime.duration.min))
                
                hybrid.cp <- sapply(which(lapply(y.list, length)>=regime.duration.min),
                                  function(i){adaptive.lasso.weighted(raw.y.list[[i]], x.list[[i]], beta.hat = beta.st[i,])})
                rownames(hybrid.cp) <- colnames(X)
                colnames(hybrid.cp) <- paste0("Regime", which(lapply(raw.y.list, length)>=regime.duration.min))
            ...
            if(ols.weight){
                ...
                hybrid.cp <- sapply(1:ns, function(i){adaptive.lasso.weighted(raw.y.list[[i]], x.list[[i]], beta.hat = beta.st[i,])})
                rownames(hybrid.cp) <- colnames(X)
                colnames(hybrid.cp) <- paste0("Regime", 1:ns)
            }
        }
        ...
        if(ols.weight){
            ...
            hybrid.dss <- matrix(adaptive.lasso.weighted(yhat, X, beta.hat = beta.st), ncol(X), 1)
            rownames(hybrid.dss) <- colnames(X)

            hybrid.cp <- matrix(adaptive.lasso.weighted(y, X, beta.hat = beta.st), ncol(X), 1)
            rownames(hybrid.cp) <- colnames(X)
