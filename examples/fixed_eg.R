...
agl.cp0 <- BridgeFixedPanel(formula=formula, data = agl, model = model, index = index, effect = effect,
                            mcmc=mcmc, , verbose=verbose, Waic = TRUE, 
                            n.break = 0)
agl.cp1 <- BridgeFixedPanel(formula=formula, data = agl, model = model, index = index, effect = effect,
                            mcmc=mcmc, , verbose=verbose, Waic = TRUE, 
                            n.break = 1)
agl.cp2 <- BridgeFixedPanel(formula=formula, data = agl, model = model, index = index, effect = effect,
                            mcmc=mcmc, , verbose=verbose, Waic = TRUE, 
                            n.break = 2)
